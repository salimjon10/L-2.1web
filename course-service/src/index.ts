...
const { port, courseServiceUrl: courseUrl, queue: courseQueue, mongoURL: dbUrl, rabbitMQUrl } = config;
...
async function connectRabbitMQ() {
    ...
    while (retries) {
        try {
            ...
            console.log('[*] Ожидает сообщения из очереди:', courseQueue);

            channel.consume(courseQueue, (msg) => processMessage(channel, msg), { noAck: false });
...
const connectDB = async (retryCount = 0) => {
    ...
    try {
        ...
        connectRabbitMQ().then(() => {
            app.listen(port, () => {
                console.log(`[x] Courses Service запущен на порту: ${port}`);
            });
        });
    } catch (error) {
