...
commentsRoute.all('/comments*', async (req: Request, res: Response) => {
	...
	try {
        ...
        try {
            ...
            await channel.assertQueue(config.commentServiceQueue, { durable: false });
            channel.sendToQueue(config.commentServiceQueue, Buffer.from(JSON.stringify(message)));

            console.log(`[x] запрос отправлен в очередь: ${config.commentServiceQueue}`);
            await channel.close();
