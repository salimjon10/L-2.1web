...
enrollmentRoute.all('/enroll*', async (req: Request, res: Response) => {
	const requestId = uuidv4();
	const path = req.originalUrl.replace('/api/enroll', '');
	...
	try {
        ...
        try {
            ...
            await channel.assertQueue(config.enrollmentServiceQueue, { durable: false });
            channel.sendToQueue(config.enrollmentServiceQueue, Buffer.from(JSON.stringify(message)));

            console.log(`[x] запрос отправлен в очередь: ${config.enrollmentServiceQueue}`);
            await channel.close();
