...
userRoute.all('/users*', async (req: Request, res: Response) => {
	...
	try {
        ...
        try {
            ...
            await channel.assertQueue(config.userServiceQueue, { durable: false });
            channel.sendToQueue(config.userServiceQueue, Buffer.from(JSON.stringify(message)));

            console.log(`[x] запрос отправлен в очередь: ${config.userServiceQueue}`);
            await channel.close();
